...
const AddDay = () => {
  ...
  const [meals, setMeals] = useState([{ mealName: '', calories: '', protein: '' }]);
  ...
  const addMealField = () => {
    setMeals([...meals, { mealName: '', calories: '', protein: '' }]);
  ...
  const handleExerciseSubmit = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/api/workout/addrecord", WorkoutRecord, {
        headers: {
  ...
  const handleMealSubmit=async()=>{
    ...
    try {
      const response = await axios.post("http://localhost:3000/api/meal/addcaloriecount", CalorieRecord, {
        headers: {
          "Content-Type": "application/json"
        }
      });
